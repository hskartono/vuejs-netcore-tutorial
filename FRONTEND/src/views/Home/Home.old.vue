  ...
  <div style="padding-left: 15px; padding-right: 15px;">
    <b-row>
        <div class="col-xs-12 col-sm-6" style="padding-left: 0px">
            <b-input-group>
                <b-form-input class="col-xs-12 col-sm-6" v-model="name" size="sm"></b-form-input>
                <b-input-group-append>
                    <b-button v-on:click="getApplicationDefinitions">
    ...
    <b-row class="mt-2" v-if="isDataLoading">
        <HomeLoadingBar v-for="item in [1, 2, 3, 4, 5, 6, 7]" v-bind:key="item"></HomeLoadingBar>
    </b-row>
    <b-row class="mt-2" v-if="!isDataLoading">
            ...
            <div class="menu-group mt-2">
                <b-card header-tag="header" footer-tag="footer">
                    <template v-slot:header>
                        <h6 class="mb-0" style="text-align:center">{{ (item != undefined && item != null ? (item.applicationName != undefined ? item.applicationName.slice(0, 13) : "No Name") : "No Name") }}</h6>
                    </template>
                    <center>
                    <img src="img/nopic.png" class="menu-icon-img" id="menu-icon-img" alt="menu icon" />
                    </center>
                    <template v-slot:footer class="p-0">
                        <b-row>
                            <b-col cols="6" style="padding-left:13px">
                                <b-button :to="{ path: '/applicationdefinition/detail/' + item.id + '?from=home' }" class="btn-first" variant="primary">My List</b-button>
                            </b-col>
                            <b-col cols="6" style="padding-left:3px">
                                <b-button :to="{ path: '/applicationdefinition/edit/' + item.id + '?from=home' }" class="btn-second" variant="primary">Edit</b-button>
                            </b-col>
                        </b-row>
                    </template>
                </b-card>
...
import HomeLoadingBar from "@/components/HomeLoadingBar";

export default {
    components : {HomeLoadingBar},
    name: 'Home',
    methods : {
      getApplicationDefinitions : function() {
        this.isDataLoading = true;
        axios
            .get(process.env.VUE_APP_API_URL + "ApplicationDefinitions",
             {
               params: {
                 name:this.name
               }
    ...
    computed: {
      ...
      dataList() 
      {
          if (this.items != undefined) {
              return this.items.slice(((this.currentPage - 1) * this.perPage),((this.currentPage - 1) * this.perPage) + this.perPage);
          }
          return [];
      }
    }
