<template>
	<div>
		...
		<div class="table-corner">
			<b-table id="my-table" stacked="md" head-variant="light"  :no-local-sorting="true" :items="purchaseRequests" :fields="fields" :busy="isDataLoading" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" small responsive>
			...
			<template v-slot:cell(actions)="row" >
				...
				<b-button size="sm" v-if="(userRole != null ? userRole.allowRead : false)" variant="primary" class="mr-2 mt-md-2 mt-lg-0 btn-xs" :to="{path: '/purchaserequest/detail/' + row.item.id }" >
...
export default {
	...
	watch: {
		...
		'$route'(to, from) {
			if (to != null) {
				if (to.name == "purchaserequestindex") {
